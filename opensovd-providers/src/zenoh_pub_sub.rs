...
impl ZenohProvider {
    ...
    pub async fn new(config: ZenohConfig) -> anyhow::Result<Self> {
        ...
        let session = zenoh::open(zenoh_config)
            ...
            .map_err(|e| anyhow!("{e}"))?;
        tracing::info!(
            "ZenohProvider successfully connected to {}",
            config.endpoint
        );
...
impl DiscoveryProvider for ZenohProvider {
    ...
    > {
        ...
        tracing::info!(
            "Starting discovery via Zenoh Liveliness Tokens with selector: {}",
        ...
        for (robot_name, points) in robot_map {
            tracing::info!(
                "SOVD component created: '{}' (via Liveliness Token)",
                robot_name
            );
            let mut component =
...
impl DataProvider for ZenohDataProvider {
    ...
    async fn read(&self, data_id: &str, _include_schema: bool) -> Result<Data, DataError> {
        ...
        match timeout(Duration::from_secs(5), subscriber.recv_async()).await {
            ...
            Err(_) => Err(DataError::NotFound(format!(
                "Timeout: Robot did not send data on '{key}'"
            ))),
        }
    }
